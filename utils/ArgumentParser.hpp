...
namespace ArgumentParser {
    ...
    inline bool parseCashierArgs(int argc, char* argv[], CashierArgs& args) {
        ...
        if (!parseKeyT(argv[1], args.shmKey)) { detail::err("Invalid shmKey"); return false; }
        if (!parseKeyT(argv[2], args.semKey)) { detail::err("Invalid semKey"); return false; }
        if (!parseKeyT(argv[3], args.cashierMsgKey)) { detail::err("Invalid cashierMsgKey"); return false; }
        return true;
    }
    ...
    inline bool parseTouristArgs(int argc, char* argv[], TouristArgs& args) {
        ...
        if (!parseUint32(argv[2], args.age))           { detail::err("Invalid age"); return false; }
        if (!parseEnum(argv[3], 0, 1, args.type))      { detail::err("Invalid type (0-1)"); return false; }
